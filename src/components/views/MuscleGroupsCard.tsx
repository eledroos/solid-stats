...
import { MuscleGroupStats } from '../../types';
...
export function MuscleGroupsCard({ stats, year, cardRef, isCapturing }: MuscleGroupsCardProps) {
  ...
  const pushPercent = stats.classesWithData > 0
    ? Math.round((stats.pushCount / stats.classesWithData) * 100)
    : 50;
  const pullPercent = 100 - pushPercent;

  // Get data for current view
  const radarData = muscleView === 'lower'
    ? Object.fromEntries(LOWER_MUSCLES.map(m => [m, stats.lowerBodyCounts[m]]))
    : Object.fromEntries(UPPER_MUSCLES.map(m => [m, stats.upperBodyCounts[m]]));

  const topMuscle = muscleView === 'lower' ? stats.topLower : stats.topUpper;
  const neglectedMuscle = muscleView === 'lower' ? stats.neglectedLower : stats.neglectedUpper;
    ...
    >
      ...
      <div className="bg-black p-4 flex justify-between items-end border-b-4 border-black">
        ...
        <div className="text-right">
          <div className="text-primary font-black text-xl">{year}</div>
      ...
      {isCapturing && (
        <div className="px-3 pb-3 bg-white border-t-2 border-gray-100">
          ...
          <div className="text-xl font-black">{stats.classesWithData}</div>
        </div>
      )}
